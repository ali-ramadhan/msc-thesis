...
Now that we have a more sophisticated method for reconstructing geometries, we should reconstruct the same geometries we saw in section \ref{sec:degenerateGeometries} for the \ch{OCS} $(2,2,2)$ molecule. We will look at the process of analyzing and plotting the \ch{OCS} $(2,2,2)$ \SI{7}{\femto\s} geometries for figure \ref{fig:OCS2227fsMOGeometry} in detail.
...
Before plotting, badly reconstructed geometries and duplicate geometries are filtered out. A badly reconstructed geometry satisfy one of three criteria. (1) It does not satisfy the KKT optimality conditions from section \ref{ssec:KKT} in which case the optimization algorithm is said to not have converged and \texttt{MultiStart} assigns such a geometry with an exit flag of $1$ making it easy to filter out such geometries. (2) It produces momentum vectors with a high absolute error when compared to the measured momentum vectors. We choose a threshold of $10^{-50}$ above which we say that the geometry is not precise enough to be a good reconstruction. The vast majority of reconstructions have significantly lower error and in general, geometries with a high error do not satisfy (1) either. (3) It lies very close to the box constraints (within $0.01$) that form our cuboid of physically realistic geometries. This usually indicates that the optimal geometry lies outside the constraints. Such bad geometries tend to also satisfy (1) and (2), but some redundancy is desirable to find all badly reconstructed geometries.

For our reconstruction, we used \texttt{MultiStart} with $50$ runs, which in hindsight was quite excessive. Out of 1,285 sets of measured momentum vectors, we attempted to reconstruct each 50 times, recovering 53,648 geometries in total. 50,615 represented duplicate geometries leaving us with 3,033 unique reconstructed geometries. 1,816 had an exit flag of 1 indicating a bad geometry (a non-optimal solution). Zero geometries with high error were found, having all been found with an exit flag of 1. Then a further 91 geometries were found very close to the box constraints. Filtering out these 1,816 + 91 = 1,907 bad geometries, we are left with 1,126 good reconstructed geometries. 1,072 measurements were mapped to a single geometry, 18 measurements to two distinct degenerate geometries, and 6 to three distinct degenerate geometries. This corresponds to 2\% of reconstructed geometries containing degeneracies. 1,096 measurements were successfully reconstructed, giving an 85\% success rate.

The good 7 fs geometries are all plotted in figure \ref{fig:OCS2227fsMOGeometry} and the others are tabulated in appendix \ref{appx:supplementaryFigures}. Reconstruction statistics for each is tabulated in table \ref{table:MOSuccess}. The modal and average geometries of for each pulse length up to 100 fs are tabulated in table \ref{table:MOGeometries}. The 200 fs data was not analyzed as the molecule seemed to have stretched way too much for the reconstruction to seem trustworthy.
...
The modal and average geometries of the two are very similar.

While the overall geometry is not much different, the absolute errors on each geometry are much smaller. The lookup table produces geometries with absolute errors on the order of $10^{-48}$ at best due to its low resolution but the optimization routine here finds geometries with errors on the order of $10^{-59} - 10^{-54}$, corresponding to an extra 3-4 decimal places of accuracy on the molecular parameters.

Some degenerate geometries.
