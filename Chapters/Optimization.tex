...
\end{equation}
where $\nabla f(x_k)$ and $H[f(x_k)]$ are the gradient and Hessian respectively, of the objective function $f$ at the point $x_k$ \citep{More83}. A quadratic function is convex and thus such a convex ``subproblem'' that locally approximates the optimization problem can be solved efficiently.
...
If the problem is locally convex, then a trust region subproblem is created and solved to take the next step. Otherwise, a direct step is taken to solve the KKT optimiality conditions.
...
\subsubsection*{Conjugate gradient step by solving a trust region subproblem}
