...
As we saw in section \ref{ssec:momentumDiscrepency}, the discrepency in the OCS \SI{7}{\fs} $(2,2,2)$ momentum vectors is less than $2\%$ and so while it will be a contributing factor to the uncertainty in the reconstructed geometries, most of the error must be due to our assumption that the molecule starts exploding from its equilibrium geometry and that each atomic fragment starts with zero initial momentum.
