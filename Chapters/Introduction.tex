...
``CEI is an effective and straightforward tool to retrieve molecular structural properties, requiring no a prior assumptions about the molecule'' \citep{Xu16}. 

``Thus, simultaneous determination of the final fragments velocity vectors from each single molecule, which is what CEI detectors do,  resolves the 3D initial conformation. Collecting conformations from an  ensemble of molecules unravels the density function of conformations of that  ensemble, conveying the detailed correlated variations within the molecular  structure''  \citep{Vager01}.

``we find a three-dimensional structure that reproduces the measured fragment velocities'' \citep{Legare05structure}.

``Coulomb explosion provides a direct access to the instantaneous structure of the target molecule'' \citep{Matsuda14}.
